...
# Ensure Calibri font is available
...
#sns.scatterplot(data=df_2100, x="cotton_withdrawal", y="cotton_scarcity", s=60, color=blue_color, alpha=1, ax=axs[0, 0], legend=False,edgecolor=(1, 1, 1, 0.5))
axs[0, 0].set_title("(a) IRB", fontdict={'fontsize': 20, 'fontweight': 'bold', 'family': 'Calibri'})
axs[0, 0].tick_params(axis='x', labelsize=16)
axs[0, 0].tick_params(axis='y', labelsize=16)
axs[0, 0].set_xlabel('')  # Remove x-axis label for the IRB plot
...
for i, method in enumerate(methods):
    ...
    #sns.scatterplot(data=df_2100, x='cotton_withdrawal', y='cotton_scarcity', hue=method, palette=palette, ax=axs[row, col], s=60, legend=False,edgecolor=(1, 1, 1, 0.5))
    axs[row, col].set_title(titles[i], fontdict={'fontsize': 20, 'fontweight': 'bold', 'family': 'Calibri'})
...
plt.tight_layout()
output_filename = "Figure_1_All_methods_IRB.png"  # Change this to your preferred file path and name
