...
for i, (cluster_std, desired_distance, desired_angle) in enumerate(zip(cluster_std_samples, desired_distance_samples, desired_angle_samples)):
    ...
    save_dataset(df, f"Case1_datasets/{transform_name}", filename)
    ...
    centroid_filepath = os.path.join(f"Case1_datasets/{transform_name}", centroid_filename)
...
output_folder_path = 'Case1_datasets/Case1_FINAL_result_pickles'
...
plt.tight_layout()
plt.savefig(f'{output_folder_path}/heatmap_OSTI_Case1.png',dpi=1200)
plt.close()
